<template>
  ...
  >
    ...
    <div class="el_cls_operation_box">
        ...
        <div v-if="!infer_status" :style="{
              ...
              opacity:
                can_click_infer
                  ? '1'
                  : '0.5',
            }" class="el_cls_inference_btn" @click="inferGo">{{ $t("TargetDetection.ButtonInferenceStart") }}</div>
        <div v-if="infer_status" class="el_cls_inference_btn" @click="inferGo">{{ $t("TargetDetection.ButtonInferenceEnd") }}</div>
        <div
          style="
            display: flex;
            flex-direction: column;
    ...
    <div class="el_cls_img_box" id="el_id_img_box">
      <canvas class="el_cls_empty_canvas" id="el_id_empty_canvas"></canvas>
      <div class="el_cls_small_img_box">
        <img
...
export default {
  ...
  watch: {
    infer_status(value){
      ...
      const response = fetch(host_ros_http1 + "/start_infer", {
        method: "POST",
        body: formData,
      });
      if(value == 0){
        this.can_click_infer = false
        const el_trained_cut_one = document.getElementById(
          "el_id_trained_cut1"
        );
        const el_trained_cut_two = document.getElementById(
          "el_id_trained_cut2"
        );
        el_trained_cut_one
          .getContext("2d")
          .clearRect(
            0,
            0,
            el_trained_cut_one.width,
            el_trained_cut_one.height
          );
        el_id_trained_cut2
          .getContext("2d")
          .clearRect(
            0,
            0,
            el_id_trained_cut2.width,
            el_id_trained_cut2.height
          );
      }
    },
    cut_status(value) {
  ...
  methods: {
    ...
    async submitRequest(tagName){
      ...
      //这里先不await了
      const response = fetch(host_ros_http1 + "/train", {
        method: "POST",
        body: formData,
      });
